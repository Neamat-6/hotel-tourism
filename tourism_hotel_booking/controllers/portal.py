...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        Booking = request.env['tourism.hotel.booking']
    ...
    def portal_my_bookings(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        return request.render("tourism_hotel_booking.portal_my_bookings", values)
    ...
    def portal_booking_page(self, booking_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            booking_sudo = self._document_check_access('tourism.hotel.booking', booking_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        if report_type in ('html', 'pdf', 'text'):
            if request.env.user.partner_id == booking_sudo.partner_id:
                report_ref = 'tourism_hotel_booking.custom_booking'
            elif request.env.user.partner_id == booking_sudo.vendor_id:
                report_ref = 'tourism_hotel_booking.vendor_custom_booking'
            else:
                report_ref = False
            return self._show_report(model=booking_sudo, report_type=report_type, report_ref=report_ref, download=download)
        ...
        values = {
            ...
            'report_type': 'html',
            'action': request.env.ref('tourism_hotel_booking.action_hotel_booking'),
        }
        ...
        return request.render('tourism_hotel_booking.booking_portal_template', values)

    # Sign Booking
    @http.route(['/my/bookings/<int:booking_id>/accept'], type='json', auth="public", website=True)
    def portal_booking_sign(self, booking_id, access_token=None, name=None, signature=None, booking_reference=None):
        ...
        if not booking_sudo.has_to_be_signed(request.env.user.partner_id):
            return {'error': _('The booking is not in a state requiring customer signature.')}
        if not signature:
            return {'error': _('Signature is missing.')}

        try:
            if booking_sudo.state == 'waiting_hotel':
                booking_sudo.write({
                    'hotel_signed_by': name,
                    'hotel_signed_on': fields.Datetime.now(),
                    'hotel_signature': signature,
                    'hotel_booking_reference': booking_reference,
                    'state': 'hotel_confirm'
                })
            else:
                booking_sudo.write({
