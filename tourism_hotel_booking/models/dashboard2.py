...
class HotelBookingDashboard(models.TransientModel):
    _name = 'tourism.hotel.booking.dashboard2'
    ...
    def button_overdue(self):
        self.ensure_one()
        action = self.env.ref('tourism_hotel_booking.action_hotel_booking').read()[0]
        overdue_booking_ids = self.env['tourism.hotel.booking'].search([('state', '!=', 'cancelled')]).filtered(lambda x: x.amount_due > 0)
    ...
    def dashboard_data_view1(self, date_from=False, date_to=False):
        booking_obj = self.env['tourism.hotel.booking']
        ...
        for room_id in room_ids:
            ...
            for date in self.get_dates_between(date_from, date_to):
                col += 1
                booking = self.env["tourism.hotel.booking"].get_booking(date=date, room_id=room_id, data=booking_data)
                ...
                if text == "BOOKED":
                    row_data += [{"class": "bsv-td-cell-booked", "body": [
                        {"type": "icon", "icon": 'fa-calendar-check-o', "class": "bsv-btn-hotel-booked-icon", "model": "tourism.hotel.booking", "res_id": booking['id']},
                        {"type": "action", "text": booking['name'], "class": "bsv-btn-hotel-booked", "model": "tourism.hotel.booking", "res_id": booking['id']},
                    ]}]
                elif text == "IN":
                    booking = booking or {"name": "---", "id": -1}
                    row_data += [
                        {"class": "bsv-td-cell-booked", "body": [{"type": "icon", "icon": 'fa-bed', "class": "bsv-btn-hotel-stay-icon", "model": "tourism.hotel.booking", "res_id": booking['id']},
                        {"type": "action", "text": booking['name'], "class": "bsv-btn-hotel-booked", "model": "tourism.hotel.booking", "res_id": booking['id']},
                    ]}]
                    ...
                    row_data += [{"class": "bsv-td-cell-schedule", "body": [
                            {"type": "action", "text": "SCHEDULE", "class": "bsv-btn-hotel-schedule", "model": "tourism.hotel.booking", "context": ctx}
                    ]}]
        ...
        #         if text == "BOOKED":
        #             row_data += [
        #                 {"type": "button_action", "name": booking['name'], "model": "tourism.hotel.booking", "res_id": booking['id'], "icon": "fa-calendar-check-o", "class": "ik-btn-hotel-booked", "col_width": COL_WIDTH,},
        #             ]
        #
        #         elif text == "IN":
        #             booking = booking or {"name": "---", "id": -1}
        #             row_data += [
        #                 {"type": "button_action", "name": booking['name'], "model": "tourism.hotel.booking", "res_id": booking['id'], "icon": "fa-bed","class": "ik-btn-hotel-stay", "col_width": COL_WIDTH,},
        #             ]
        #         else:
        #
        #             booking_datetime = datetime(year=date.year, month=date.month, day=date.day, hour=0, minute=0, second=0) + timedelta(hours=check_in_point_utc)
        ...
        #
        #             row_data += [
        #                 {"type": "button_action", "name": "SCHEDULE", "class": "ik-btn-hotel-schedule", "model": "tourism.hotel.booking", "context":ctx, "col_width": COL_WIDTH,},
        #             ]
        #
        #     screen.append(row_data)
        #
    ...
    def booking_data_by_room_type(self, calendar_date, hotel_id):
        ...
        for room_type in room_types:
            ...
            for room_id in self.get_rooms_sorted(hotel_id=hotel_id):
                ...
                count += 1
                booking_data = self.sudo().env["tourism.hotel.booking"].get_booking_data(date_from=calendar_date,
                ...
                for col in row:
                    ...
                    if col and calendar_date in date_list:
                        ...
                        row_data += [{"class": "bsv-td-cell", "body": [
                                {"type": "action", "text": col, "class": "bsv-btn-calendar-day", "model": "tourism.hotel.booking", "context": ctx},
                                {"type": "action", "text": text.strip() or "", "class": status_class, "model": "tourism.hotel.booking", "context": ctx},
                            ]}]
                    elif col:
