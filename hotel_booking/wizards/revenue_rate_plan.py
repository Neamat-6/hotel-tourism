...
class ReceivedRoom(models.TransientModel):
    _name = 'revenue.rate.plan.wizard'
    ...
    rate_plan = fields.Many2many('hotel.rate.plan')
    line_ids = fields.One2many('revenue.rate.plan.wizard.line', 'wizard_id')
    ...
    def get_booking_folios(self):
        ...
        if self.rate_plan:
            domain.append(('rate_plan', 'in', self.rate_plan.ids))
        if self.partner_id:
        ...
        for folio in folio_lines:
            if folio_lines:
                self.env['revenue.rate.plan.wizard.line'].create({
                    'wizard_id': self.id,
                    'booking_id': folio.booking_id.id,
                    'partner_id': folio.partner_id.id,
                    'hotel_id': folio.hotel_id.id,
                    'rate_plan': folio.rate_plan.id,
                    'state': folio.state,
                    'day': folio.day,
                    'amount': folio.amount
                })
        return {
            ...
            'name': _('Revenue Rate Plan'),
            'view_mode': 'form',
            'view_type': 'form',
            'res_model': 'revenue.rate.plan.wizard',
            'res_id': self.id,
            'target': 'new'
        }

    def print_pdf(self):
        return self.env.ref('hotel_booking.revenue_rate_plan_wizard_action_report').with_context(
            landscape=True).report_action(self)


class BookingArrivalLine(models.TransientModel):
    _name = 'revenue.rate.plan.wizard.line'

    wizard_id = fields.Many2one('revenue.rate.plan.wizard')

    day = fields.Date()
    booking_id = fields.Many2one('hotel.booking')
    partner_id = fields.Many2one('res.partner', 'Customer')
    hotel_id = fields.Many2one('hotel.hotel', 'Related Hotel')
    rate_plan = fields.Many2one('hotel.rate.plan')
