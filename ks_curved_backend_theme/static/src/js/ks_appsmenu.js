odoo.define("ks_curved_backend_theme.ks_appsmenu", function (require) {
  ...
  patch(NavBar.prototype, "ks_curved_backend_theme.ks_appsmenu", {
    setup() {
      this._super();
      useListener(
        "keydown",
        ".ks_menu_search_box input",
        this._ksSearchValuesMovement
      );
      useListener(
        "input",
        ".ks_menu_search_box input",
        this._ksSearchMenuListTime
      );
      useListener(
        "click",
        ".ks-menu-search-value",
        this._ksSearchValuesSelecter
      );
      ...
      useListener("click", ".o_app", this._onKsAppsMenuItemClicked);

      var today = new Date();
      var curHr = today.getHours();
      var message = _t("Hi, ");
      if (curHr < 12) {
        message = _t("Good Morning, ");
      } else if (curHr < 18) {
        message = _t("Good Afternoon, ");
      } else {
        message = _t("Good Evening, ");
      }
