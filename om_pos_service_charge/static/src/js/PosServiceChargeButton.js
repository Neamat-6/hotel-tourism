odoo.define("pos_order_to_purchase_order.PosServiceChargeButton", function (require) {
    ...
    class PosServiceChargeButton extends PosComponent {
        ...
        async onClick() {
            ...
            var service_charge_base = this.env.pos.config.service_charge_base;
            ...
            if (service_amount_type == 'percentage'){
                ...
                if (confirmed){
                   if (service_charge_base == 'total') {
                       console.log("total");
                       console.log(order.get_total_with_tax());
                       var service_charge =  payload / 100.0 * order.get_total_with_tax();
                   }
                   else if (service_charge_base == 'subtotal') {
                       console.log("subtotal");
                       console.log(order.get_total_without_tax());
