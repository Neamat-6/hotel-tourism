...
class AccountPayment(models.Model):
    ...
    def _onchange_journal_payment_type(self):
        ...
        if self.is_advance_payment:
            self.advance_payment_account_id = self.env['account.account'].sudo().search(
                [('used_for_advance_payment', '=', True), ('company_id', '=', self.env.company.id)], limit=1).id
    ...
    def _seek_for_lines(self):
        ...
        if not counterpart_lines and writeoff_lines:
            advance_payment_lines = writeoff_lines.filtered(
                lambda l: l.account_id.used_for_advance_payment and l.account_id.reconcile)
