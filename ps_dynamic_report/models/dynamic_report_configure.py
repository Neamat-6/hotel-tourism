...
class DynamicReportConfigure(models.Model):
    ...
    def get_names(self, model, field_name, ids):
    ...
    def create_dynamic_pdf_report(self, server):
        ...
        Heading_values.insert(0, "#")

        record_data = []
        total_data = []
        active_model_id = self.env[self._context.get('active_model')].browse(self._context.get('active_ids'))
        conditions = self._context.get('active_domain')
        print(conditions)
        domain = []

        for condition in conditions:
            name = active_model_id._fields[condition[0]].string
            if active_model_id._fields[condition[0]].type == 'many2many' or active_model_id._fields[
                condition[0]].type == 'many2one':
                field_name = condition[0]
                ids = condition[2]  # Sample IDs
                names = self.get_names(active_model_id._fields[condition[0]].comodel_name, field_name, ids)
                domain.append({
                    name: names
                })
            else:
        ...
        for model in active_model_id:
            temp = [sequence_number]  # Start with the sequence number
            sequence_number += 1  # Increment sequence number for the next line
        ...
        if server_id.dynamic_report_id.report_type == 'pdf':
            ...
            style1 = xlwt.easyxf(
                "pattern: pattern solid, fore_colour custom_colour; alignment: vert centre, horiz center; font: name Arial, bold True;")
            style2 = xlwt.easyxf(
                "pattern: pattern solid, fore_colour silver_ega; alignment: horizontal center; font: name Arial, bold True;")
    ...
    def create_server_action(self):
        ...
        vals = {
            ...
            'code': "action = env['dynamic.report.configure'].create_dynamic_pdf_report(server={server_id})".format(
                server_id=self.id)
        }
