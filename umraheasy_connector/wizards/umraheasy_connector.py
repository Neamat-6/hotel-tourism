...
class UmraheasyConnector(models.TransientModel):
    _name = 'umraheasy.connector'
    _description = 'Umraheasy Connector'

    line_ids = fields.One2many('umraheasy.connector.line', 'connector_id')
    ...
    def button_search(self):
        ...
        return {
            ...
            'name': _('Umraheasy Connector'),
            'view_mode': 'form',
            'view_type': 'form',
            'res_model': 'umraheasy.connector',
            'res_id': self.id,
            'target': 'current'
        }

    def button_update_inventory(self):
        url = "https://live.umraheasy.com/api/v2/cm/update/hotels-task"

        data = {
            "hotelCode": self.company_id.related_hotel_id.umraheasy_code,
                ...
                "rooms": [
                    {
                        ...
                        "roomCode": line.room_type_id.umraheasy_code
                    }
                ]
            })
        return vals
    ...
    def get_rate(self):
        ...
        while start <= end:
            if self.rate_plan_ids:
                rate_plans = self.rate_plan_ids.filtered(lambda p: p.umraheasy_code)
            else:
                rate_plans = self.env['hotel.rate.plan'].search([
                    ('company_id', '=', self.company_id.id), ('umraheasy_code', '!=', False)
                ])
    ...
    def button_update_rate(self):
        url = "https://live.umraheasy.com/api/v2/cm/update-rates/hotels-task"
        data = {
            "hotelCode": self.company_id.related_hotel_id.umraheasy_code,
    ...
    def prepare_rates(self):
        vals = []
        for line in self.line_ids.filtered(lambda l: l.rate_plan_id.umraheasy_code):
            date = line.date.strftime('%Y-%m-%d')
            vals.append({
                    "startDate": date,
                    "endDate": date,
                    "rates": [
                        {
                            "roomCode": line.room_type_id.umraheasy_code,
                            "rate": line.rate,
                            "rateplanCode": line.rate_plan_id.umraheasy_code
                        }
                    ]
                })
        return vals
...
class UmraheasyConnectorLine(models.TransientModel):
    _name = 'umraheasy.connector.line'
    _description = 'Umraheasy Connector Line'

    connector_id = fields.Many2one('umraheasy.connector')
