...
class UmraheasyController(http.Controller):

    @http.route('/umraheasy/update_reservation', auth='none', csrf=False, type='json', methods=['POST'])
    def update_reservation(self):
        ...
        hotel = request.env['hotel.hotel'].sudo().search([
            ('enable_umraheasy', '=', True), ('umraheasy_code', '=', args["hotelCode"])
        ])
        ...
        if args['action'] == 'book':
            ...
            booking_id = request.env['hotel.booking'].sudo().search([
                ('umraheasy_booking_ref', '=', args['bookingId']), ('state', '!=', 'cancelled')
            ])
    ...
    def create_booking(self, args, company):
        ...
        booking_id = request.env['hotel.booking'].sudo().with_company(company).create({
            ...
            'hotel_id': hotel.id,
            'umraheasy_channel': args.get('channel', False),
            'umraheasy_booking_ref': args.get('bookingId', False),
            'umraheasy_cm_booking_ref': args.get('cmBookingId', False),
            'umraheasy_booking_date': args.get('bookedOn', False),
            'check_in': checkin,
            'check_out': checkout,
            'total_nights': total_nights,
            'umraheasy_segment': args.get('segment', False),
            'note': args.get('specialRequests', False),
            'umraheasy_pah': args.get('pah', False),
            'booking_source': 'online_agent',
            'online_travel_agent_source': company.related_hotel_id.umraheasy_partner_id.id or False,
            'ref': args.get('bookingId', False),
            'reservation_type': request.env['booking.type'].sudo().search([], limit=1).id,
            'partner_id': partner_id,
            'umraheasy_apply_daily_price': True,
    ...
    def update_booking(self, args, company, booking_id):
        ...
        booking_id.sudo().with_company(company).write({
            ...
            'umraheasy_channel': args.get('channel', booking_id.umraheasy_channel),
            'umraheasy_booking_ref': args.get('bookingId', booking_id.umraheasy_booking_ref),
            'umraheasy_cm_booking_ref': args.get('cmBookingId', booking_id.umraheasy_cm_booking_ref),
            'check_in': checkin,
            'check_out': checkout,
            'total_nights': total_nights,
            'umraheasy_segment': args.get('segment', booking_id.umraheasy_segment),
            'note': args.get('specialRequests', booking_id.note),
            'umraheasy_pah': args.get('pah', booking_id.umraheasy_pah),
            'partner_id': partner_id,
            'line_ids': lines
        })
        return booking_id
    ...
    def get_rate_plan_room_type(self, room_dict, company):
        ...
        room_type = request.env['room.type'].sudo().search([
            ('company_id', '=', company.id), ('umraheasy_code', '=', room_dict['roomCode'])
        ])
        if not room_type:
            return {
                "status": 403,
        ...
        rate_plan = request.env['hotel.rate.plan'].sudo().search([
            ('company_id', '=', company.id), ('umraheasy_code', '=', room_dict['rateplanCode'])
        ])
        if not rate_plan:
            return {
                "status": 403,
