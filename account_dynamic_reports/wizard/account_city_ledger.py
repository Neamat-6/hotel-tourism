...
class AccountCityLedger(models.TransientModel):
    _name = "account.city.ledger"
    ...
    account_ids = fields.Many2many(
        ...
        default=lambda self: self.env['account.account'].search([('is_filtered_account', '=', True)])
    )
    journal_ids = fields.Many2many(
        'account.journal', string='Journals',
        default=lambda self: self.env['account.journal'].search([('is_city_ledger', '=', True)], limit=1)
    )
    ...
    def create(self, vals):
        ret = super(AccountCityLedger, self).create(vals)
        return ret
    ...
    def write(self, vals):
        ...
        if not vals.get('journal_ids'):
            vals.update({'journal_ids': self.env['account.journal'].search([('is_city_ledger', '=', True)]).ids})
        # if vals.get('journal_ids'):
        #     vals.update({'journal_ids': vals.get('journal_ids')})
        # if vals.get('journal_ids') == []:
        #     vals.update({'journal_ids': [(5,)]})
        if not vals.get('account_ids'):
            vals.update({'account_ids': self.env['account.account'].search([('is_filtered_account', '=', True)]).ids})
        ...
        ret = super(AccountCityLedger, self).write(vals)
        return ret
    ...
    def get_filters(self, default_filters={}):
        ...
        partner_company_domain = [('parent_id', '=', False),
                                  ...
                                  ('company_id', '=', self.env.company.id),
                                  ('company_id', '=', False)]
        account_account_domain = [('company_id', '=', self.env.company.id)]
        journals = self.journal_ids if self.journal_ids else self.env['account.journal'].search(
            [('is_city_ledger', '=', True), ('company_id', '=', self.env.company.id)])
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            ...
            '.action_print_city_ledger').with_context(landscape=True).report_action(
    ...
    def action_xlsx(self):
        ...
        sheet = workbook.add_worksheet('City Ledger')
        sheet.set_zoom(95)
        sheet_2 = workbook.add_worksheet('Filters')
        sheet_2.protect()

        # Get record and data
        record = self.env['account.city.ledger'].browse(data.get('id', [])) or False
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 8, 'City Ledger' + ' - ' + data['company_id'][1], format_title)
        ...
        report_id = self.env['common.xlsx.out'].sudo().create({'filedata': result, 'filename': 'CityLedger.xls'})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=common.xlsx.out&field=filedata&id=%s&filename=%s.xls' % (
                report_id.id, 'City Ledger.xls'),
            'target': 'new',
        }
