...
//    _("Redirected"),
//    message: "Window has been redirected",
//});
...
var DynamicClMain = AbstractAction.extend({
template:'DynamicClMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
'type': 'ir.actions.report',
'report_type': 'qweb-pdf',
'report_name': 'account_dynamic_reports.city_ledger',
'report_file': 'account_dynamic_reports.city_ledger',
'data': {'js_data':data},
'context': {'active_model':'account.city.ledger',
'landscape':1,
'from_js': true
},
'display_name': 'City Ledger',
};
return self.do_action(action);
});
},
...
if(initial_render){
self.$('.py-control-panel').html(QWeb.render('FilterSectionCl', {
filter_data : datas[0],
}));
self.$el.find('#date_from').datepicker({ dateFormat: 'dd-mm-yy' });
self.$el.find('#date_to').datepicker({ dateFormat: 'dd-mm-yy' });
...
cl_lines_by_page : function(offset, account_id){
var self = this;
return self._rpc({
model: 'account.city.ledger',
method: 'build_detailed_move_lines',
...
var total_rows = parseInt($(event.currentTarget).data('count'));
self.loader_disable_ui();
self.cl_lines_by_page(offset, account_id).then(function(datas){
_.each(datas[2], function (k, v){
var formatOptions = {
currency_id: k.company_currency_id,
noSymbol: true,
...
self.plot_data(self.initial_render);
});},});core.action_registry.add('dynamic.fr', DynamicFrMain);core.action_registry.add('dynamic.gl', DynamicGlMain);core.action_registry.add('dynamic.pa', DynamicPaMain);core.action_registry.add('dynamic.pl', DynamicPlMain);core.action_registry.add('dynamic.cl', DynamicClMain);core.action_registry.add('dynamic.tb', DynamicTbMain);
