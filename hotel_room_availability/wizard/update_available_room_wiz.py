        ...
        for rec in self:
            ...
            for line in rec.room_qty_line_ids:
                for i in date_list:
                    ...
                    ttl_qty = line_rec.room_qty #or line.hotel_room_id.rooms_qty
                    ...
                    print('ttl_qty =====>  ', ttl_qty)
                    print('line_rec.contract_qty =====>  ', line_rec.contract_qty)
                    print('line_rec =====>  ', line_rec)

                    if (line_rec.contract_qty + ttl_qty) < 0 :
                        raise ValidationError(_(" Available Qty Must Be Positive "))

                    if (ttl_qty*-1) > line_rec.contract_qty:
                        raise ValidationError(_
                                              ("The room %s can not be updated. The maximum quantity is %d" % (line.hotel_room_id.name, line_rec.contract_qty*-1)))

                    if line_rec:
                        line_rec.update({'room_qty': ttl_qty})
                        date = i.date()
                        contract_qty =line_rec.contract_qty
                        room_qty = ttl_qty
                        user_id = self.env.uid
                        room_id = line.hotel_room_id.id
                        contract_line = self.env['hotel.contract.line'].search(
                            [('room_type', '=', room_id), ('start_date', '<=', date), ('end_date', '>=', date)],
                            limit=1)
                        if contract_line:
                            self.env['hotel.contract.log.line'].create(
                                {'log_id': contract_line.contract_id.id, 'user_id': user_id, 'room_id': room_id,
                                 'date': date, 'count': contract_qty + room_qty})
...
class UpdateAvailableRoomQtyLine(models.TransientModel):
    ...
    def value_negative(self):
        ...
        for recs in self:
            if recs.value > 0:
                raise ValidationError(_("Value should not be positive"))

    room_qty_id = fields.Many2one('update.available.room.qty', string="Update Room Qty")
    hotel_id = fields.Many2one('hotel.hotel', string='Hotel', related='room_qty_id.hotel_id', store=True)
