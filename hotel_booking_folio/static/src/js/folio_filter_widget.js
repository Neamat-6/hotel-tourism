odoo.define('hotel_booking_folio.folio_filter_widgets', function (require) {
	"use strict";
	...
	ListRenderer.include({
		_updateSelection: function () {
	        ...
	        console.log('length>>', this.selection.length)
    ...
    var One2manyFolioFilter = FieldOne2Many.extend({
		template: 'One2manyFolioFilter',
		events: {
			"click #button_checkin_order_lines": "check_in_selected_lines",
			"click #button_checkout_order_lines": "check_out_selected_lines",
			"click .button_select_order_lines": "selected_lines",
		},
		init: function() {
	        this._super.apply(this, arguments);
	    },
		check_in_selected_lines: function(){
			var self=this;
			...
			var w_response = confirm("Dou You Want to Check In ?");
			if (w_response) {
                ...
                rpc.query({
                    ...
                    if (result['error'] !== undefined){
                        return self.displayNotification({ message: result['error'], type: 'danger' });
                    }
                    self.trigger_up('reload');
                });
             }
		},
		check_out_selected_lines: function(){
			var self=this;
			var current_model = this.recordData[this.name].model;
			var selected_lines = self.find_deleted_lines();
			if (selected_lines.length === 0)
			...
			var w_response = confirm("Dou You Want to Check Out ?");
			if (w_response) {
	...
	fieldRegistry.add('one2many_folio_filter', One2manyFolioFilter);
});