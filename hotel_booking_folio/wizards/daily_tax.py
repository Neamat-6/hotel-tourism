...
class DailyTax(models.TransientModel):
    _name = 'daily.tax'
    _description = 'Daily Revenue'

    line_ids = fields.One2many('daily.tax.line', 'wizard_id')
        ...
        while start <= end:
            ...
            for partner in partners:
                ...
                if self.filter_service:
                    total_service = abs(sum(lines.filtered(lambda l: l.type not in ['room_charge', 'tax', 'discount'] and not l.payment_id).mapped('amount')))
                total = total_room_charge + total_vat + total_municipality
                if total:
                    self.env['daily.tax.line'].create({
                        'wizard_id': self.id,
...
class DailyTaxLine(models.TransientModel):
    _name = 'daily.tax.line'
    _description = 'Daily Tax Line'

    wizard_id = fields.Many2one('daily.tax')
