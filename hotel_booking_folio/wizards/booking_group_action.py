...
class BookingGroupAction(models.TransientModel):
    ...
    def button_auto_assign(self):
        ...
        for i in range(start, end + 1):
            ...
            # floor = self.env['hotel.floor'].search([('company_id', '=', company_id), ('id', '=', self.floor_end.id)])
            print('Floor:', floor)
            for folio in self.folio_ids:
                if folio.id in folios:
                    floor_rooms = floor.room_ids.filtered(
                        lambda r: r.room_type.id == folio.room_type_id.id and r.stay_state.id == vacant
                    )
                    print('Floor Rooms:', floor_rooms)
                    if self.assign_clean_room:
                        floor_rooms = floor_rooms.filtered(lambda r: r.state.id == clean)
                        print('Clean Rooms:', floor_rooms)
                    intersection = list(set(floor_rooms.ids) & set(folio.get_available_rooms()))
                    ...
                    if intersection:
                        ...
                        if old_room_id:
                            ...
                            if folio.check_in_date == folio.company_id.audit_date:
                                print('Check In Date audit:', folio.check_in_date)
                                new_room_id.stay_state = self.env.ref(
                                    'hotel_booking.data_hotel_room_stay_status_arrival').id
                            else:
                                print('Check In Date:', folio.check_in_date)
                        ...
                        folio.room_id = intersection[0]
                        print('Folio Room:', folio.room_id)
